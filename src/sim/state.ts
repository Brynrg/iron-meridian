...
export function spawnUnit(state: SimState, type: string, owner: number, x: number, y: number, facing = 0): Actor {
  ...
  if (def.harvester) a.harvester = { cargo: 0, gems: 0, state: "idle", refinery: -1, fieldX: -1, fieldY: -1 };
...
export function findExitCell(state: SimState, producer: Actor, loco: Actor["loco"]): [number, number] {
  ...
  for (const [x, y] of candidates) {
    ...
    const t = m.terrain[i] as Terrain;
    if (loco === "naval" ? t === Terrain.Water : t !== Terrain.Water && t !== Terrain.Cliff && t !== Terrain.Tree && t !== Terrain.Rock) return [x, y];
  }
  return [producer.tx, producer.ty + producer.h];
}
