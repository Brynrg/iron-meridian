...
function think(state: SimState, p: Player, out: Command[]): void {
  ...
  if (bq.items.length === 0 && !bq.ready) {
    const order = BUILD_ORDER[p.faction] ?? [];
  ...
  const threshold = (p.difficulty === "hard" ? 6 : p.difficulty === "easy" ? 14 : 9) + Math.floor(state.tick / (20 * 240));
  const underAttack = state.tick - mem.lastBaseAttackTick < 20 * 15;

  if (underAttack) {
    // Defend: everyone to the last hit location.
    const ids = army.map((a) => a.id);
    if (ids.length && state.tick % 40 === 0) out.push({ player: p.id, kind: "attackMove", actors: ids, x: mem.targetX, y: mem.targetY, queue: false });
    mem.attacking = false;
    return;
  }
  if (!mem.attacking) {
    ...
    if (state.tick % 100 === 0) {
      const target = pickTarget(state, p, conyard);
      const idle = army.filter((a) => a.order.kind === "idle");
...
function placeReady(state: SimState, p: Player, conyard: Actor, out: Command[]): void {
  const mem = p.ai;
  if (!mem) return;
...
function findSpot(state: SimState, p: Player, type: string, conyard: Actor, defense: boolean): [number, number] | null {
  ...
  for (let r = start; r <= 14; r++) {
    ...
    for (let i = 0; i < ring.length; i++) {
      ...
      if (canPlace(state, p.id, type, tx, ty) && laneClear(state, tx, ty, w, h)) return [tx, ty];
    }
  }
  return null;
}
